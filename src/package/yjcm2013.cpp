        ...
        if (use.card->isNDTrick() || use.card->isKindOf("BasicCard")) {
            ...
            if (choice == "cancel")
                ...
                if (!use.card->isKindOf("Collateral"))
                    ...
                    if (extra == NULL) {
                        extra = available_targets.at(QRandomGenerator::global()->generate() % available_targets.length() - 1);
                        ...
                        extra->tag["collateralVictim"] = QVariant::fromValue((victims.at(QRandomGenerator::global()->generate() % victims.length() - 1)));
                    }
                }
                use.to.append(extra);
