            ...
            if (!target->isKongcheng()) {
                ...
                if (target->getMark(objectName()) < num && !target->isKongcheng()) {
                    ...
                    forever {
                        ...
                        int give = QRandomGenerator::global()->generate() % rest_num + 1;
                        rest_num -= give;
                        QList<int> to_give = handcard_list.length() < give ? handcard_list : handcard_list.mid(0, give);
                        ServerPlayer *receiver = room->getOtherPlayers(target).at(QRandomGenerator::global()->generate() % (target->aliveCount() - 1));
...
public:
    ...
    bool onPhaseChange(ServerPlayer *target) const
    {
        if (target->getPhase() == Player::Start && target->askForSkillInvoke(this)) {
            ...
            if (c == NULL) {
                c = target->getCards("he").at(QRandomGenerator::global()->generate() % target->getCardCount());
...
void OlRendeCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const
{
    ...
    QStringList tmp = source->property("olrende").toString().split("+");
    QSet<QString> rende_prop(tmp.begin(), tmp.end());
