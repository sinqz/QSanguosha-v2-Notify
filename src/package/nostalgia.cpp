    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == HpRecover) {
            ...
            if (recover.who && recover.who != player) {
                room->broadcastSkillInvoke("nosenyuan", QRandomGenerator::global()->generate() % 2 + 1);
            ...
            if (source && source != player) {
                room->broadcastSkillInvoke("nosenyuan", QRandomGenerator::global()->generate() % 2 + 3);
                room->sendCompulsoryTriggerLog(player, objectName());
        ...
        if (triggerEvent == EventPhaseStart && shuangying->getPhase() == Player::Draw && TriggerSkill::triggerable(shuangying)) {
            if (shuangying->askForSkillInvoke(this)) {
                ...
                if (diff) {
                    ...
                    room->broadcastSkillInvoke(objectName(), QRandomGenerator::global()->generate() % 2 + 1);
        ...
        if (use.card->isKindOf("Slash")) {
            ...
            if (target) {
                ...
                if (target->getHandcardNum() > 1) {
                    ...
                    if (!card)
                        card = target->getHandcards().at(QRandomGenerator::global()->generate() % target->getHandcardNum());
                } else {
