...
public:
    ...
    {
        int index = QRandomGenerator::global()->generate() % 2 + 1;
...
public:
    ...
    {
        QStringList tmp = Sanguosha->getLimitedGeneralNames();
        QSet<QString> all(tmp.begin(), tmp.end());
        Room *room = zuoci->getRoom();
        if (isNormalGameMode(room->getMode())
            || room->getMode().contains("_mini_")
            || room->getMode() == "custom_scenario") {
            tmp = Config.value("Banlist/Roles", "").toStringList();
            all.subtract(QSet<QString>(tmp.begin(), tmp.end()));
        } else if (room->getMode() == "06_XMode") {
            foreach(ServerPlayer *p, room->getAlivePlayers()) {
                tmp = p->tag["XModeBackup"].toStringList();
                all.subtract(QSet<QString>(tmp.begin(), tmp.end()));
            }
        } else if (room->getMode() == "02_1v1") {
            tmp = Config.value("Banlist/1v1", "").toStringList();
            all.subtract(QSet<QString>(tmp.begin(), tmp.end()));
            foreach(ServerPlayer *p, room->getAlivePlayers()) {
                tmp = p->tag["1v1Arrange"].toStringList();
                all.subtract(QSet<QString>(tmp.begin(), tmp.end()));
            }
        }
