...
QString TrustAI::askForKingdom()
{
    ...
    if (!lord) return kingdoms.at(QRandomGenerator::global()->generate() % kingdoms.length());

    switch (self->getRoleEnum()) {
    case Player::Lord: role = kingdoms.at(QRandomGenerator::global()->generate() % kingdoms.length()); break;
