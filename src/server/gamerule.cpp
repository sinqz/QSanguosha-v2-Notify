    ...
    if (level <= Config.BossLevel - 1) {
        ...
        if (boss_generals.length() == 1)
            ...
            if (Config.value("OptionalBoss", false).toBool())
                ...
                general = boss_generals.at(QRandomGenerator::global()->generate() % boss_generals.length());
        }
    } else {
        general = (QRandomGenerator::global()->generate() % 2 == 0) ? "sujiang" : "sujiangf";
    }
...
void GameRule::acquireBossSkills(ServerPlayer *player, int level) const
{
    ...
    for (int i = 0; i < num; i++) {
        QString skill = skills.at(QRandomGenerator::global()->generate() % skills.length());
        skills.removeOne(skill);
        if (skill.contains("+")) {
            QStringList subskills = skill.split("+");
            skill = subskills.at(QRandomGenerator::global()->generate() % subskills.length());
        }
